...
**Status**: Done, including the multi-item follow-up. Base integration implemented in PR [#5045](https://github.com/mrwulf/home-cluster/pull/5045); multi-leg flight grouping, multiple-bookings-per-email, the scoped cruise-stops Ollama enrichment (§3.1.1), and two real bugs found along the way (QDateTime-wrapped dates, multi-passenger leg grouping) shipped in PR [#5046](https://github.com/mrwulf/home-cluster/pull/5046). Live-tested end to end throughout — see [§8](#8-acceptance-checklist), every item checked.
...
**Current live state**: `cluster/apps/household/n8n/app/workflows/trip-ingest.json` calls `mcp-kitinerary`'s `extract_booking` first (new `Kitinerary Extract` node), maps every reservation it returns (connecting flight legs pre-grouped into one booking each), and fans out into one Trek booking per mapped item (`Trek Resolve` mode: `runOnceForEachItem`). Ollama only runs as a fallback when kitinerary returns nothing usable, plus one narrow supplementary call for cruise stops (§3.1.1). `mcp-kitinerary`'s ToolHive proxy turned out to speak a **stateless** per-request MCP variant (`params._meta` envelope), not the stateful `initialize`+session pattern `Trek Resolve` uses — see [§2](#2-before-writing-any-workflow-code-verify-the-new-servers-own-contract), confirmed live before any workflow code was written.
...
**Status: built.** `Kitinerary Extract` groups connecting `FlightReservation` legs (same reservation number, arrival airport of leg N === departure airport of leg N+1, gap under 24h and forward in time) into ONE `flight` `ex` item, with the intermediate connection airports in `stops[]` (the same field already used for cruise ports), matching Trek's own single-multi-leg-booking behavior. Any reservations that don't group (genuinely separate bookings — a non-connecting outbound+return, or a hotel + rental car in one itinerary PDF) each become their own `ex` item; `Code in JavaScript1` fans these out into separate n8n items and `Trek Resolve` (mode: `runOnceForEachItem`) creates one Trek booking per item, verified live for both the grouped and separately-fanned-out cases.

**Multi-passenger wrinkle found along the way**: kitinerary commonly emits one near-identical `FlightReservation` per passenger per leg, all sharing the same confirmation number. Grouped naively, this can chain leg N of one passenger's itinerary onto leg N+1 of a _different_ passenger's purely because the times line up — confirmed live with a synthetic 2-passenger, 2-leg fixture (3 garbled groups instead of 1 correct one) before being fixed by deduping identical legs (same route + times) before grouping.

**Two datetime bugs found and fixed while building this**: kitinerary wraps some (not all) datetime fields as `{"@type":"QDateTime","@value":"...","timezone":"..."}` rather than a plain ISO string — `FlightReservation` departure/arrival times are always wrapped once an airport is timezone-resolved, while `LodgingReservation` check-in/out times came back as plain strings for an otherwise-equivalent source shape. Unwrapped, this silently produced `NaN` in the grouping's date math (so connecting legs were never recognized as connecting) and an object where Trek Resolve expected a string (`"Invalid time value"` crash) — already live in PR #5045 for any real flight/train/bus booking, since that PR's own live test only exercised lodging. Fixed by extracting `.@value` wherever a date field is read from the raw kitinerary object, not just where it was observed broken.

### 3.1.1 Should kitinerary also call Ollama for a second opinion?

Considered and **rejected as a blanket policy**: always also asking Ollama after a kitinerary match would reintroduce LLM latency and hallucination risk on the cases kitinerary already handles completely and deterministically (lodging, flight, the majority of real confirmation emails) — for no benefit on those cases.

**Adopted narrowly instead**: kitinerary structurally cannot extract multi-port cruise itineraries (`BoatReservation` has no per-port `stops` in schema.org/kitinerary's model at all — confirmed live). `Code in JavaScript1` makes one targeted supplementary Ollama call, reusing the existing cruise few-shot prompt, _only_ when a mapped item is `booking_type: 'cruise'` with empty `stops` — and only harvests `.stops` from the result, keeping every other kitinerary-derived field (dates, confirmation code, amount) as the deterministic source of truth. Non-fatal on failure (the booking still lands, just without port-by-port stops). Verified live: the enrichment call fires only for this one case, integrates cleanly, and doesn't block the booking — though the actual port-extraction _quality_ is subject to the same run-to-run Ollama variance already documented and accepted elsewhere in this pipeline (§8.6 in the master doc), not a new limitation this introduces.
...
- **A Code node in `mode: "runOnceForEachItem"` disallows `$input.first()`/`$input.all()` entirely** (confirmed live: `Can't use .first() here [line 1, for item 0]` / `This is only available in 'Run Once for All Items' mode`) — use `$json` (current item) or `$('OtherNode').first()` (cross-node reference, still valid in each-item mode) instead. The final `return` must also be a bare `{ json: {...} }`, not `[{ json: {...} }]` — the array form throws `A 'json' property isn't an object [item 0]`. This is exactly why `Trek Resolve` switched to this mode carefully, verified first against a disposable duplicate n8n workflow (§6's methodology), not the live one.
- **n8n's each-item-mode validator does a naive text scan for disallowed method names, not AST analysis** — a code _comment_ that literally contains the text `.first()` gets the node rejected (`Can't use .first() here`) even though no actual call exists. Caught mid-session: a comment explaining the `$input.first()` restriction itself tripped the same restriction.
- **When inspecting a `runOnceForEachItem` node's execution data via the API, `runData["NodeName"]` is always length 1** regardless of how many items it processed — the per-item results live inside `runData["NodeName"][0].data.main[0]` as an array, one entry per item. Checking `.length` on the outer array (easy mistake, matches the shape for `runOnceForAllItems` nodes) looks exactly like "only the first item ran" and will send you chasing a bug that isn't there — confirmed by chasing exactly that phantom bug for a while before realizing the inspection code, not the pipeline, was wrong.
- **kitinerary-extractor's own result validation rejects an under-specified reservation outright** (`items: []`, not a partial extraction) — a synthetic `BoatReservation` fixture with only `reservationFor.name`/`departureTime`/`arrivalTime` (no `departureBoatTerminal`/`arrivalBoatTerminal`) produced nothing at all; adding proper `BoatTerminal` objects with address data made it extract correctly. Worth knowing when a real cruise confirmation email's embedded JSON-LD is minimal — kitinerary may find nothing rather than something partial, and the pipeline correctly falls through to Ollama in that case (verified: this is exactly what execution 71 in this session's testing did).
