...
**A real bug found live**: a genuine resend silently vanished — no n8n execution, no bounce visible to the sender. Cloudflare's `message.from` is the SMTP envelope sender, and the household mail relay rewrites it into a VERP bounce-tracking address (`bounce+<hash>-plans=<domain>@<relay-domain>`) for some forwarding paths — which matches no sender-allowlist rule and is neither a deliverable inbox nor a Trek account, even though the message's own `From:` header still carried the real sender. The Worker (a separate repo, [`cloudflare-implementation`](https://github.com/mrwulf/cloudflare-implementation)) now also parses the raw message's own `From:` header as a second trust signal, and prefers it (when present) over the envelope for the `X-Envelope-From` value forwarded to n8n — that's the address n8n actually needs to be reachable.
...
1. `list_trips`, then resolve a target trip in priority order:
   - **An LLM read of the forwarder's own note, if it expresses one** (see below) — either a specific trip or a forced new trip, bypassing date matching entirely.
   - **Exact containment** — a trip whose own `start_date`/`end_date` already spans the booking's start date always wins, regardless of list order.
   - **Fuzzy fallback** — only when no trip already contains the date: the closest trip within a ±1 day window of the booking's start date (nearest by gap, not first-in-list). This ranking exists because a bare "first trip whose fuzzy window matches" previously let a short, nearby trip steal a booking that belonged to a longer trip already covering the date — see git history for the exact incident. The window was tightened from an earlier ±3 days once the exact-containment check above made the wide fuzz mostly redundant and occasionally over-eager.
   - No match at all → `create_trip`.
...
**Forward-note trip suggestion.** The person forwarding a booking can steer trip resolution in plain language, written anywhere in their own note above the quoted confirmation — no fixed syntax to remember (e.g. "this one's for the Miami trip in October" or "make this its own trip"). `extractForwardNote` in `Trek Resolve` cuts the email's `bodyPreview` (the cleaned plain-text body the ingestion node already extracts) off at the first sign of forwarded/quoted material — original-message markers, `Begin forwarded message:`, an Outlook `From:`/`Sent:` header block, `On ... wrote:`, or `>`-quoted lines — so only the human-written note is ever shown to a model, never the receipt itself. When there's a note and at least one existing trip, `suggestTrip` sends that note plus each trip's id/title/date range to Ollama (`qwen3.5:4b`, a small model since this is a one-shot classification over a handful of options, not extraction) with a JSON schema forcing one of: attach to a specific trip id, force a new trip, or no preference. A trip id the model names that doesn't actually exist falls back to normal date matching and says so in the notification; any call failure (timeout, bad JSON, model unavailable) does the same, silently — this is a convenience layer on top of date matching, not a replacement for it. No note, or no existing trips to choose from, skips the call entirely.

### 5. Notification

One email per created/duplicate/failed booking (n8n's default per-item behavior for a non-Code node downstream of a per-item execution) via the cluster's existing outbound relay — no new SMTP credential. Every notification ends with a `Ref: n8n-exec-<id>` correlation ID, a direct key into n8n's own execution history for diagnosis. A failure anywhere in the chain (extraction, Trek write) still produces a notification — the pipeline never fails silently. The `Trip:` line carries the trip's title alongside its link (not just the bare URL).
...
- [x] Trip matching/creation via Trek's native MCP: exact date containment first, then closest trip within a ±1 day window
- [x] Trip auto-expansion when a booking falls outside the matched trip's date range
- [x] Forward-note trip suggestion (LLM reads the forwarder's own plain-language note) to override automatic trip matching
- [x] Trip title shown alongside the trip link in every notification email
