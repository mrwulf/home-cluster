...
- [x] ~~Handle `message/rfc822` MIME encapsulation (double-forwarded emails)~~ — was silently dropped inside `walkMime` (no branch matched it, so it was neither parsed as multipart nor captured as an attachment). Fixed with a branch that recurses into the part's body as a full nested email (`splitHeaderBody` + `walkMime` again). Also switched `collector.textPlain`/`textHtml` from first-wins scalars to arrays that get joined — a forwarder's short note ("FYI, see attached") was otherwise winning the first-match guard and shadowing the actual booking content sitting in the nested forwarded message, defeating the point of parsing it at all. Verified live (execution 34, isolated test trip 167, deleted after): a `message/rfc822`-forwarded booking email produced a `bodyPreview` containing both the forwarder's note and the nested email's full content, and Ollama correctly extracted the complete flight booking (`FWDTEST1`) from it.
- [x] ~~Process all PDF attachments, not just the first~~ — was `collector.attachments.find(...)` (first PDF only) feeding a single `binary.attachment` property. Fixed by collecting every PDF-matching attachment and fanning out one n8n item per PDF from the MIME-parser node; "Extract PDF Text" runs per-item automatically, and the Ollama node now aggregates all extracted texts (`$input.all()`, each capped at 4000 chars, 12000 total) into one combined prompt instead of reading only the first. Verified live (execution 33, isolated test trip 166, deleted after): a two-PDF email fanned out to 2 items, each extracted independently by "Extract PDF Text," and the resulting booking was correctly created from the combined text.
...
- [x] ~~Treat Ollama's fallback `{booking_type: 'general', parse_error}` output as a failure worth telling the sender about~~ — was silently proceeding to create a junk "general" Trek reservation. Fixed: right after parsing, the Ollama node now validates the result — `parse_error` set, `booking_type` missing/outside the known enum, or an unparseable `start_datetime`/`end_datetime` all set `extractionError`, routing to the existing "could not process" failure email instead of Trek. Unit-tested standalone (6 cases: valid flight, `parse_error`, bad enum, missing `booking_type`, unparseable date, all-null-dates-valid — all passed) and live-regression-tested (execution 36, isolated test trip 169, deleted after) to confirm a legitimate booking still passes validation cleanly. The actual trigger (Ollama really returning garbage) wasn't fault-injected live, same caveat as other Ollama-path fixes.
- [x] ~~Detect and flag PDFs with no extractable text (scanned image, no text layer)~~ — `extractFromFile` succeeds with empty text on a scanned/no-text-layer PDF, and Ollama was extracting from whatever remained with no signal anywhere. Fixed: the Ollama node now counts how many attached PDFs actually yielded text vs. how many were attached; a shortfall sets `pdfExtractionWarning`, which Trek Resolve appends as a `Warning:` line in the notification email (success, duplicate, and Trek-failure branches) so the sender knows to double-check. Verified live (execution 35, isolated test trip 168, deleted after): one real PDF plus one hand-built valid-but-textless PDF (blank content stream, confirmed with `pdftotext` to yield zero text) produced `pdfExtractionWarning: "1 of 2 attached PDF(s) had no extractable text..."`, and the actual sent notification email included that warning line.
- [x] ~~Investigate the Cloudflare Worker's behavior when the webhook fetch fails~~ — read `handleTripIngest` in `cloudflare-implementation/worker.js`. It does **not** silently drop: a `fetch` throw (unreachable, DNS, cert) or a non-2xx response both call `message.setReject(...)` with a specific reason string, which Cloudflare's Email Workers turns into an SMTP-level rejection of the _inbound_ transaction — the original sender's own mail server shows them a bounce/NDR containing that reason text. There's no retry on the Worker's side (`catch` is immediate, one attempt), and no explicit timeout is set on that `fetch` call (no `AbortController`, no override in `wrangler.jsonc` beyond `compatibility_date`), so it runs to whatever Cloudflare's platform default subrequest timeout is — not confirmed against current Cloudflare docs/dashboard, see the _next item_.
- [x] ~~Worker/n8n timeout-mismatch risk~~ — `Respond Success` previously only fired after Ollama + all Trek MCP calls + SMTP send completed inside one synchronous execution; this session's retry-with-backoff work raised the worst case to ~720s+, and Cloudflare's docs don't specify an Email Worker duration limit (checked `developers.cloudflare.com/workers/platform/limits/` directly — no subrequest timeout while the client stays connected, no Email Worker section at all), so the real ceiling was an unverified guess at best, most likely the sending mail server's own SMTP DATA timeout. Decided with the user to fix the root cause rather than just shrink the window or leave it flagged: `Respond Success` now fires immediately after the secret-validity gate (the only fast-path check that exists) instead of at the end of the chain, and the same n8n execution continues into PDF extraction, Ollama, Trek, and Notify afterward — n8n's `responseMode: "responseNode"` supports this natively; a `Respond to Webhook` node sends the HTTP response wherever it sits in the graph and downstream nodes keep executing regardless. Trade-off: only the secret check can still reject the SMTP transaction with a bounce; Ollama/Trek failures rely entirely on the notification email now, not an SMTP-level signal, since the response has already gone out by the time those run. Verified live: response latency dropped to ~0.4s (execution 37) with the full async chain (`Extract PDF Text` → `Code in JavaScript1` → `Trek Resolve` → `Notify`) still completing to `status: success` a few seconds later, real Trek booking created; bad-secret requests still get a synchronous 401 (unchanged, that branch wasn't touched).
...
- [x] ~~Lodging dedup fallback (`overlapping stay dates`) compared Trek's opaque `start_day_id`/`end_day_id` values directly as if they were date-ordered~~ — false-positive duplicate reported by the user (2026-08-31, `n8n-exec-39`): a Comfort Inn & Suites Harrisonburg booking, for which Ollama couldn't extract a confirmation code, fell past the exact-confirmation-match branch into this fallback and was matched against an unrelated existing Best Western Plus Savannah Gateway accommodation despite the two stays having non-overlapping real dates. Root cause: Trek's MCP exposes a standalone `create_day` tool ("add a day to a trip with optional date"), meaning day rows are database records created as needed rather than pre-generated in chronological order per trip — `day.id` is an opaque key, not a date-ordered index — but the fallback was calling `overlaps(startDayId, endDayId, a.start_day_id, a.end_day_id)` on the raw ids. Fixed by building a `day.id → day.date` map from `get_trip_summary`'s `days[]` and comparing the resolved calendar dates as epoch ms instead, mirroring the transport-type branch immediately below it (which already compared real `reservation_time`/`reservation_end_time` values, not ids). **Verified live** (2026-08-31, temporary direct PUT of the fix to the live n8n workflow via its API, reverted afterward — see the two entries below for the full live-test methodology): a real accommodation (Hampton Inn & Suites Richmond, found via `search_place`/`create_accommodation`, isolated test trip 175) followed by a second forward for the same hotel with overlapping dates and no confirmation code was correctly flagged `trekDuplicateMatchReason: "overlapping stay dates"` and created no second accommodation — this is the literal code path from the original bug report. Test trip 175 deleted via `delete_trip` immediately after.
- [x] ~~A booking outside the matched trip's current day range (an extra night, a longer drive home) silently lost its Trek day linkage instead of extending the trip~~ — found while reviewing the fix above, in response to the user asking whether the pipeline extends a trip organically as new bookings roll in. Root cause: `create_trip` auto-generates day rows only for its own `start_date`/`end_date` span, and the pipeline never called Trek's `create_day` or `update_trip` tools, so a date outside that original span had no day row. `findDayId` returned `null`, and every creation branch guards its day args with `if (startDayId) ...` — for lodging specifically, `isLodging && startDayId && endDayId` being false meant the entire `search_place → create_place → create_accommodation` path was skipped in favor of a bare `create_reservation` (`type: 'hotel'`) with no place lookup, no map pin, and (since `startDayId` was still null) no day linkage at all; flights/transit/cruise/activities/etc. lost only the day linkage, keeping their own `reservation_time`. Fixed two ways: (1) when an incoming booking matches an existing trip but its date falls outside that trip's `start_date`/`end_date`, `update_trip` now extends the trip's recorded range before `get_trip_summary` is fetched, so the trip's own metadata (and future `list_trips` window-matching) stays accurate instead of frozen at whichever booking created it; (2) a new `ensureDayId` helper (scoped inside the `!isDuplicate` branch, so a duplicate never creates orphan days) creates the missing day via `create_day` on demand before any of the type-specific branches run, so every booking type gets proper day linkage regardless of when it arrives relative to the trip's original span. Also broadened both lodging dedup checks (exact confirmation match and the overlapping-stay-dates fallback) to additionally scan `existingReservations` where `type === 'hotel'`, since a lodging booking downgraded to a plain reservation (still possible when `search_place` finds no match) previously lived outside the `existingAccommodations` scan and could evade duplicate detection on a later resend.

**Verified live** (2026-08-31): temporarily PUT the fixed workflow directly onto the running n8n instance (workflow `VPRsjZRxTBmywAdW`) via its API — bypassing GitOps for the test window only — sent hand-built raw-MIME test emails straight at the live `/webhook/trip-ingest` endpoint with the real shared secret, then inspected results via n8n's Executions API and Trek's MCP directly (`get_trip_summary`), all in isolated fictitious test trips deleted via `delete_trip` immediately after. The 15-minute `n8n-workflow-sync` CronJob had to be worked around by re-pushing the fix and completing each round of tests inside a single 15-minute window, since it silently reverts the live workflow to the git-deployed version on its own schedule — it clobbered the fix mid-test once, which was caught because the affected execution's result didn't match expectations.

This surfaced two things the wiki-only schema research (§ two entries up) got wrong or missed, both fixed and re-verified in the same live pass:

1. Trek's `day` objects _do_ carry a real `date` field once the trip has valid dates (confirmed via a direct `get_trip_summary` call) — the `day.id → day.date` map approach was sound. `update_trip` and `create_day` both behave exactly as their wiki descriptions state: extending a matched trip's `start_date`/`end_date` and adding a day with a specific date both worked on the first live call, confirmed by inspecting the trip and its `days[]` afterward (a booking dated one day past the original 3-day trip window correctly extended `end_date` and added days with the right `date`/`day_number`).
2. The reservation-side half of the broadened overlap dedup (added in the entry above) initially still missed a real duplicate: the `create_reservation` fallback (hit when `search_place` finds no place match) never populated `reservation_time`/`reservation_end_time`, so the dedup check reading those fields had nothing to compare — fixed by setting both from `ex.start_datetime`/`ex.end_datetime` in that branch. The first attempt at that fix then checked the reservation's single `day_id` _before_ the newly-populated `reservation_time` range, which under-matched a multi-night stay to just its check-in day and missed a real overlap — fixed by preferring `reservation_time`/`reservation_end_time` (the full accurate stay) and falling back to `day_id` only for older records that predate this fix. Verified live end-to-end afterward: a fictitious "QA Test Inn" booking (isolated test trip 176, no real place match so it hit the `create_reservation` fallback) followed by a same-hotel forward with overlapping dates and no confirmation code was correctly flagged `trekDuplicateMatchReason: "overlapping stay dates"`. Test trip 176 deleted via `delete_trip` immediately after.

- [x] ~~Overlapping-dates dedup fallback matched a real new booking against a completely unrelated existing record~~ — real production failure reported by the user after the fixes above were merged and deployed (2026-08-31, `n8n-exec-55`, live trip 1 "20th Birthday Photo Shoot"): a genuine second Comfort Inn & Suites Harrisonburg stay (a return-trip stop, Aug 31 15:00 → Sep 1 11:00, real confirmation `72078629417110`, distinct from the trip's first Comfort Inn stay days earlier) was rejected as a duplicate of a reservation titled "Home" — a manually-tracked, confirmation-less placeholder marking arrival-home day (Sep 1), sharing nothing with the new booking except that Sep 1 falls inside both. The real booking was silently never created in Trek. Root cause, confirmed by pulling the trip's actual data read-only via Trek's MCP and replaying the exact dedup logic offline against it: the overlap fallback (both the lodging and, symmetrically, the transport-type branch) had no correlation requirement between the new booking's provider and the existing candidate's name — bare type + date overlap was treated as sufficient, so any hotel-typed record on file with a touching date, however unrelated, could match. The user then confirmed the intended behavior directly: different hotels, transportation, or activities on the same or overlapping dates are expected and must not be merged (multiple rooms, multiple travelers, multiple things happening at once). Fixed by requiring a `providerNamesRelated` check (case-insensitive shared-word match between `ex.provider_name` and the candidate's name/title) before date overlap is consulted at all, on every overlap fallback (lodging-accommodation, lodging-reservation, and transport). An interim version of this fix instead allowed the overlap check to proceed whenever the _existing_ record had any confirmation number on file (regardless of whether it matched or even related to the new booking) — that version was caught before merging by an offline harness replaying the real Trek data plus a `Hilton Garden Inn` vs. `Marriott Courtyard`, same-dates, no-confirmation scenario, which it incorrectly flagged as a duplicate; dropping the confirmation-presence bypass and requiring the name check unconditionally fixed both the real "Home" case and that synthetic one without disturbing the two previously-verified genuine-duplicate scenarios (§ two entries up). Trade-off, accepted per the user's explicit priority: this narrows the item-587 cross-confirmation-system fallback (agency confirmation vs. cruise-line confirmation for the same real booking, names sharing no words) to no longer catch cases where the names truly share nothing — a missed duplicate there is a one-`delete_transport` fix, judged far cheaper than silently dropping a real booking. Verified via a standalone Node harness (not a live push, given trip 1 is real household data) seeded with the actual trip 1 accommodations/reservations/days pulled read-only from Trek: the real exec-55 booking now correctly resolves to "not a duplicate," the two previously-verified duplicate scenarios (same-hotel resend, QA Test Inn overlap) still correctly match, and two new different-provider-same-dates cases (lodging and flight) correctly do not match. Re-verified against the live pipeline once deployed: the user resent the real Expedia confirmation (`n8n-exec-56`), which correctly created a new accommodation ("Comfort Inn & Suites", place-resolved to 262 Neff Avenue, Harrisonburg, confirmation `72078629417110`, days 5-6) instead of matching "Home" — confirmed via a follow-up `get_trip_summary` read against the live trip.

- [x] ~~Accommodation bookings weren't assigned to each day of the stay, and Trek's check-in/check-out time and notes fields went unused~~ — user-requested enhancement after the fixes above: a created accommodation was linked to the trip via `start_day_id`/`end_day_id` but never actually appeared on any individual day's itinerary (`assign_place_to_day` was never called), making day-by-day transportation planning harder; `create_accommodation`'s `check_in`/`check_out` fields (confirmed via a live `tools/list` call against Trek's MCP, not the wiki, after being burned twice by wiki-only assumptions above) were never populated even though the extracted booking already carries the times; and the Ollama extraction prompt never asked for room/bed type or smoking preference, so it was never captured even though the `notes` field was already wired through to `create_accommodation`. Fixed: (1) `ensureDayId`/`create_day` now also captures `day_number` so a newly-created day can be placed in date order; (2) after a successful `create_accommodation`, the code now calls `assign_place_to_day` once per day from the stay's start to end day number (inclusive), sourced from `get_trip_summary`'s `days[]`; (3) `check_in`/`check_out` (`"HH:MM"` strings) are sliced directly from `ex.start_datetime`/`ex.end_datetime`'s time portion, avoiding any timezone-conversion bug that `toISOString()` would introduce on a bare local-time ISO string; (4) the Ollama system prompt now explicitly asks for room type/bed configuration/smoking preference into `notes` for lodging bookings, with the same "never invent one if not stated" guard as every other field. Verified live end-to-end via a temporary second n8n workflow (`Trip Ingest TEST - delete me`, distinct webhook path `trip-ingest-test`, same workflow content) rather than overwriting the live one, avoiding the 15-minute sync-CronJob clobbering problem from earlier live tests entirely — the user's own suggestion. A real Hampton Inn & Suites Richmond booking (isolated test trip, deleted after) with "Room Type: 2 Queen Beds, Non-Smoking" in the source email correctly extracted that into `notes`, and the created accommodation showed `check_in: "15:00"`, `check_out: "11:00"`, and all three days of the stay (check-in through check-out) each carrying a real `assign_place_to_day` assignment to the hotel, confirmed via `get_trip_summary`. Test trip and the temporary test workflow both deleted immediately after.

- [x] ~~Deterministic extraction from structured `schema.org` JSON-LD, ahead of the Ollama LLM call~~ — most real airline/hotel confirmation emails already embed the same `schema.org` `Reservation` JSON-LD Gmail parses for its own "smart" trip cards, and KDE's `kitinerary-extractor` reads it deterministically (plus structured PDF tickets and Wallet passes the MIME/regex extraction never touched). Full design and build history live in two companion docs rather than duplicated here: [kitinerary_mcp_server_spec.md](kitinerary_mcp_server_spec.md) (the standalone MCP server, published to `ghcr.io/mrwulf/kitinerary-mcp`, registered via PR [#5044](https://github.com/mrwulf/home-cluster/pull/5044)) and [trip_ingest_kitinerary_integration_spec.md](trip_ingest_kitinerary_integration_spec.md) (the workflow wiring, PR [#5045](https://github.com/mrwulf/home-cluster/pull/5045)). Both are done and live-verified — see that doc's own §8 acceptance checklist for the full evidence trail (execution 62: 7ms kitinerary match, real Trek trip/reservation/cost confirmed via direct DB read; execution 63: Ollama fallback regression-tested clean, 18.8s real LLM round-trip). Trek's own REST booking-import API was evaluated and rejected first — its login-session JWT requirement is structurally incompatible with the `trek_`-prefixed MCP token this pipeline holds (see kitinerary_mcp_server_spec.md §0), which is exactly why this runs as its own small MCP server rather than calling Trek's built-in feature.
...
5. ~~PDF with no text layer: succeeds with empty text, no signal.~~ **Fixed** — see checklist above (`pdfExtractionWarning`).
...
7. ~~SMTP relay down when sending the notification email: the sender is never told either way.~~ **Partially fixed** — the `Notify` node now has native n8n `retryOnFail` (3 tries, 5s apart) plus `onError: continueRegularOutput`, so a transient relay blip self-heals and a still-down relay no longer hard-fails the execution or blocks the webhook response. Doesn't fully close the gap: if SMTP is down for the full retry window, the sender genuinely can't be told by email (physical limitation), and there's still no separate "booking succeeded, notification failed" signal recorded anywhere for the admin to catch later — n8n's own execution history is the only record in that case. Not live fault-injection-tested (would mean disrupting the shared smtp-relay), same caveat as the Ollama/Trek retry work.
8. ~~No retry anywhere in the chain (Ollama call, every Trek MCP call, SMTP send)~~ — **fixed**: Ollama and every Trek MCP call retry 3x with exponential backoff; SMTP send now retries natively via the Notify node's `retryOnFail` (item 7).
9. No dead-letter/reprocess mechanism (see checklist above).

Remaining open items: malformed/schema-violating Ollama output silently creating a junk reservation (item 4), Worker webhook-failure behavior (unverified, upstream of this pipeline), the Worker/n8n timeout-mismatch risk, and no dead-letter/reprocess mechanism.
