...
- [ ] **Milestone 1**: Confirm/rotate the Trek API token used for MCP access; store it as `API_TOKEN` on the existing "Trek Service Credentials" Bitwarden item (extends `trek`'s current ExternalSecret, doesn't replace it).
- [ ] **Milestone 2**: Add the `plans@${SECRET_DOMAIN}` routing rule + trigger match to the existing Cloudflare Worker; provision `TRIP_INGEST_SHARED_SECRET` on both the Worker (`wrangler secret put`) and the cluster (`n8n-trip-ingest-secrets` ExternalSecret).
- [ ] **Milestone 3**: Develop and test the Ollama extraction prompt & JSON schema against the test email corpus; empirically validate latency on the Intel-iGPU `ollama` deployment and pick the final model size.
- [x] **Milestone 4 (partial)**: Built and verified the n8n orchestration workflow's ingestion half — webhook (raw MIME, POST, `/webhook/trip-ingest`) → shared-secret gate (Code node + IF, with a proper 401 on mismatch) → basic MIME field extraction (From/Subject/body via regex, not a real parser). **Not yet built**: Ollama extraction, Trek MCP resolver, `smtp-relay` notifier. n8n 2.37.4 has no native MCP Client node in this install; calling `/mcp` via raw HTTP is the fallback, still unverified.
- [x] **Milestone 5**: Stood up the n8n Public API sync as a CronJob (§4.3, not the originally-sketched Job — see rationale there) and proved the round-trip: edit `workflows/trip-ingest.json` in git → `PUT` applies it → n8n reflects the change, webhook still resolves correctly, without hand-editing in the n8n UI.
- [ ] **Milestone 6**: Export declarative n8n workflow into GitOps repo and validate with `mise x -- task test:all`.
