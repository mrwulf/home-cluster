...
- [ ] Wrap the Ollama call in retry-with-backoff (2-3 attempts) — currently single-attempt; a transient restart now reaches the failure email (see resolved items below) instead of dying silently, but still costs the sender a manual resubmit.
- [ ] Wrap each Trek MCP call in retry-with-backoff similarly — same single-attempt situation; failures now notify instead of dying, but still no automatic retry.
...
8. No retry anywhere in the chain (Ollama call, every Trek MCP call, SMTP send) — a single transient blip still fails that run's actual goal (getting the booking into Trek), even though the sender is now told about it instead of hearing nothing. Still open.
9. No dead-letter/reprocess mechanism (see checklist above).

Rough priority order for closing what's left: retry-with-backoff on the Ollama and Trek MCP calls (most home-lab failures are transient pod restarts, and now that failures notify instead of vanishing, retrying before giving up avoids bothering the sender for a blip that would've cleared itself), then a "processing succeeded but notification failed" path so a Trek booking is never created without the sender eventually finding out.
