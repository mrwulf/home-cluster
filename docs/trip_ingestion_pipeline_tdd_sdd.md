...
- **Trek accommodation/lodging bookings are simplified.** `booking_type: lodging` currently goes through `create_reservation` with `type: hotel` and a bare `day_id`, not the full `search_place → create_place → create_accommodation` flow the original design (§3.3) describes — that needs real place resolution (geocoding/POI search) which wasn't built. Flights and generic transit go through `create_transport` correctly.
