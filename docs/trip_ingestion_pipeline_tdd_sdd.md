...
- [ ] **Worker/n8n timeout-mismatch risk — heightened by this session's own retry work, needs a decision.** `Respond Success` only fires after Ollama + all Trek MCP calls + SMTP send complete inside one synchronous n8n execution. Before this session, worst case was one 240s Ollama call; the retry-with-backoff fix above (justified on its own) raises that to up to 3 attempts (~720s) plus Trek's own retries on top. If Cloudflare's actual subrequest timeout for this Worker is shorter than n8n's worst-case run time — unverified, needs checking against Cloudflare's current docs/dashboard for this account's plan — the Worker's `fetch` throws, the sender gets a "webhook is unreachable" bounce, **and n8n likely keeps running to completion regardless** (nothing here cancels workflow execution when the calling connection drops), later sending its own "Added to Trek" success email. The sender would see a bounce and a success email disagreeing with each other for the same message. Fixing this properly means decoupling the webhook response from the Ollama/Trek work (n8n "respond immediately" + continue processing, or split into two workflows) — a real design change to the response contract the Worker depends on, not a same-shape patch like the other items here. Deliberately left for an explicit decision rather than solo cross-repo architecture change.
