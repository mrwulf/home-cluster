...
- **Storage**: one PVC. VolSync-backed (`templates/volsync/primary` component + `scripts/stagger-volsync.py`) if the data is worth restoring after a rebuild; a plain `PersistentVolumeClaim` — with a rule-8 comment explaining why — only for genuinely disposable/cache state.
...
- **`MCPServer`'s top-level `spec.volumes` only supports `hostPath`** (not node-portable, don't use it for real storage). A PVC-backed volume needs `podTemplateSpec.spec.volumes[].persistentVolumeClaim` + a matching `volumeMounts` entry on the container named `mcp`.
