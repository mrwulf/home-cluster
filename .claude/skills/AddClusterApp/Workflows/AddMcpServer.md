...
5. **Storage**, if any: default to `podTemplateSpec.spec.volumes[].emptyDir: {}` for disposable/cache state — mounted on the container named `mcp`. Example: `cluster/apps/ai/toolhive/servers/mcp-pullmd.yaml`. Only add a `PersistentVolumeClaim` (referenced the same way via `persistentVolumeClaim.claimName`, not VolSync unless the data is worth backing up) when a cold `emptyDir` would meaningfully hurt startup time or the volume must be shared across replicas. Top-level `spec.volumes` on `MCPServer` only supports `hostPath` — don't use it for either case.
