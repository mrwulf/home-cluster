...
1. Create base64 encoded Redis configuation
   ...
   ```sh
    # Create base64 encoded Redis configuration
    echo -n \
      '{"db":15,"sentinels":[
        {"host":"redis-node-0.redis-headless.databases.svc.cluster.local","port":26379},
        {"host":"redis-node-1.redis-headless.databases.svc.cluster.local","port":26379},
        {"host":"redis-node-2.redis-headless.databases.svc.cluster.local","port":26379}
      ],"name":"redis-master"}' \
      | base64 -w 0
   ```
...
2. Use this base64 encoded string in the Kubernetes secret
   ...
   ```yaml
   REDIS_URL: ioredis://<base64-encoded-config>
   ```
